...
static CGPoint _previousLocation;
...
jstring Java_com_phatware_android_RecoInterface_WritePadAPI_recognizeTestFile( JNIEnv * env, jobject thiz, jbyteArray buff )
{
    // Testing recognizer using saved ink file (5 different configurations)
    jstring         result = NULL;
    jboolean    isCopy = JNI_FALSE;
    ...
    *resultstr = 0;
    for ( i = 0; i < 5; i++ )
    {
        recognizedText = recognizeInkData( iData, i );
        if ( recognizedText && *recognizedText )
        {
            strcat( resultstr, recognizedText );
        }
    }
    
    if ( buffer && isCopy == JNI_TRUE)
        (*env)->ReleaseByteArrayElements(env, buff, buffer, JNI_ABORT);
